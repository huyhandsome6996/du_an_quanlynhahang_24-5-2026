...
│   │   └── NguoiDung.cs             (Bản vẽ tài khoản đăng nhập - mật khẩu plain-text)
│   │
...
│   │   ├── ChiTietHoaDonDAL.cs      (Thủ kho chuyên quản lý từng dòng món trong Hóa đơn)
│   │   ├── NguoiDungDAL.cs          (Thủ kho chuyên quản lý tài khoản đăng nhập)
│   │   └── Interfaces/              (Bảng nội quy quy định các thủ kho phải làm gì)
│   │
│   ├── CacModun/                    <-- KHU VỰC "PHÒNG BẾP" (Các modun logic)
│   │   ├── KhoiTaoUngDung.cs        (Cấu hình DI + CORS + JSON + kiểm tra CSDL)
│   │   ├── KhoiDongDesktop.cs       (Mở cửa sổ WinForms + WebView2)
│   │   ├── ApiTaiKhoan.cs           (Endpoint Đăng ký / Đăng nhập)
│   │   ├── ApiBan.cs                (Endpoint CRUD Bàn + Đặt bàn)
│   │   ├── ApiSanPham.cs            (Endpoint CRUD Sản phẩm)
│   │   ├── ApiHoaDon.cs             (Endpoint Mở bàn / Gọi món / Thanh toán)
│   │   └── ApiBaoCao.cs             (Endpoint Báo cáo doanh thu + Top món bán chạy)
│   │
